...
class PostsControls{
    ...
    private function createLikeButton($postId,$postedBy,$user_id){
        $text = $this->posts->getLikes($postId);
        $class = "like-btn";
        $imageSrc ='<i class="fa fa-heart-o"></i>';
        $action = "likePost(this,$postId,$user_id,$postedBy);";
        if($this->posts->wasLikedBy($user_id,$postId)){
            $imageSrc ='<i class="fa fa-heart"></i>';
        }
        
        return '<div class="postButtonContainer">
                '.ButtonProvider::createLikeButton($text,$imageSrc,$class,$action,$postId,$postedBy,$user_id).'
                </div>';
    }
}
